...
class SimConfigRL:
    ...
    use_partial_observability: bool = True

@dataclass
class EnvConfig:
    n_animals: int = 40
    n_drones: int = 4
    dt: float = 0.2                  # coarser physics for RL speed (~4x faster)
    decision_interval: int = 5       # 1 RL step = 5 sim steps (1 second)
    max_decisions: int = 150         # episode ends after 150 decisions (150s sim time)
...
class TrainConfig:
    ...
    save_path: str = "models/gains_ppo"
    log_path: str = "logs/gains_ppo"

@dataclass
class CurriculumConfig:
    """Curriculum only varies animal count"""
